...
export default function NotificationsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const data = await notificationsApi.list({ date, sectionId, status });
      setRows(Array.isArray(data) ? data : []);
    } catch (e) { setErr(e?.message ?? "Error al cargar."); }
    finally { setLoading(false); }
  }, [canQuery, date, sectionId, status]);

  const sendAll = useCallback(async () => {
    if (!canQuery) return;
    setLoading(true); setErr("");
    try { await notificationsApi.sendAbsences({ date, sectionId }); await load(); }
    catch (e) { setErr(e?.message ?? "Error al enviar."); }
    finally { setLoading(false); }
  }, [canQuery, date, sectionId, load]);

  const resend = useCallback(async (id) => {
    setLoading(true); setErr("");
    try { await notificationsApi.resend(id); await load(); }
    catch (e) { setErr(e?.message ?? "Error al reintentar."); }
    finally { setLoading(false); }
  }, [load]);
  ...
  const testSend = useCallback(async () => {
    ...
    } catch (e) { setErr(e?.message ?? "Error al enviar prueba."); }
    finally { setLoading(false); }
  }, [testTo, testMsg]);
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex flex-wrap items-center gap-2 mb-3">
        ...
        <input type="number" value={sectionId} placeholder="Sección" onChange={e => setSectionId(+e.target.value)} className="border rounded px-2 py-1 w-28"/>
        ...
        <button onClick={sendAll} disabled={!canQuery || loading} className="bg-green-600 text-white px-3 py-1 rounded disabled:opacity-50">Enviar ausentes</button>
        <button onClick={load} disabled={!canQuery || loading} className="bg-slate-600 text-white px-3 py-1 rounded disabled:opacity-50">Refrescar</button>
      </div>
        ...
        {rows.map(n => (
          <div key={n.id} className="border rounded p-3 bg-white">
            <div className="flex justify-between">
              ...
              <span className={`text-xs px-2 py-0.5 rounded ${n.status === "SENT" ? "bg-green-100 text-green-700" : n.status === "FAILED" ? "bg-red-100 text-red-700" : "bg-amber-100 text-amber-700"}`}>{n.status}</span>
            </div>
            ...
            {n.status === "FAILED" && (
              <button onClick={() => resend(n.id)} disabled={loading} className="mt-2 text-sm bg-blue-600 text-white px-2 py-1 rounded disabled:opacity-50">
                Reintentar
              </button>
            )}
          </div>
        ))}
        {rows.length === 0 && !loading && canQuery && <div className="text-sm text-slate-500">Sin mensajes.</div>}
      </div>
    </div>
  );
}